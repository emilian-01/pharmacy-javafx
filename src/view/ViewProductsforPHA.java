...
public class ViewProductsforPHA {
	//private Product prod;
	private int type = 0;
	private Employee emp;
	public ViewProductsforPHA() {	}
	...
	public void show(Stage st) {
		...
		hb.getChildren().addAll(sell,/*add, edit, del,*/ back);
		hb.setSpacing(10);
...
*/
		sell.setOnAction(e->{
			for (Object x : table.getSelectionModel().getSelectedItems()) {
				if (x instanceof Product)
					(new SellProduct(((Product) x).getName(), ((Product) x).getCategory(),
							((Product) x).getSupplier(), ((Product) x).getQuantity(),
							((Product) x).getPrice(), ((Product) x).getExpDate())).show(st);
			}
			for (Object x : table.getSelectionModel().getSelectedItems()) {
